...
# Define Reidemeister move operators as unitary transformations
...
for potential_type in ptypes:
    ...
    H_free = - np.diag(np.ones(n_x-1), -1) - np.diag(np.ones(n_x-1), 1)  # Kinetic term
    H_free *= -1 / (2 * dx**2)
    ...
    ax.legend()
    # Time evolution function for animation
    n_steps = 2000
    frames = 2000  # Number of frames in animation
    # Time evolution function
    def update(frame):
        global psi
        for _ in range(n_steps // frames):
            psi = U @ psi
            if np.random.uniform(0,1) < 0.1:  # Apply random Reidemeister moves
                psi = apply_reidemeister_sequence(psi, [np.random.choice([R1, R2, R3])])
            psi /= np.linalg.norm(psi)  # Normalize to avoid numerical instability
    ...
    ani.save(f"animation_qt_rei_{potential_type}.mp4", writer="ffmpeg", fps=60)
